...
function getAllTownshipEntries() {
  const list = [];
  for (const [county, names] of Object.entries(TAIWAN_TOWNSHIPS)) {
    for (const name of names) list.push({ name, county });
  }
  return list;
}
...
const _allEntries = getAllTownshipEntries();
const _nameMap = new Map(); // key -> array of { name, county }
function addKey(key, entry) {
  key = normalizeInput(key);
  if (!_nameMap.has(key)) _nameMap.set(key, []);
  const arr = _nameMap.get(key);
  if (!arr.some(e => e.name === entry.name && e.county === entry.county)) arr.push(entry);
}
_allEntries.forEach(({ name, county }) => {
  const entry = { name, county };
  addKey(name, entry);
  const last = name.slice(-1);
  if (last === "區" || last === "鄉" || last === "鎮" || last === "市") {
    addKey(name.slice(0, -1), entry);
  }
});
_allEntries.forEach(({ name, county }) => {
  const key = normalizeInput(name).replace(/台/g, "臺");
  if (!_nameMap.has(key)) _nameMap.set(key, []);
  const arr = _nameMap.get(key);
  if (!arr.some(e => e.name === name && e.county === county)) arr.push({ name, county });
});
