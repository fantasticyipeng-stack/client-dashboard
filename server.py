...
def seed_defaults():
    """確保預設客戶與剪輯手一定存在（缺誰補誰）；可重複呼叫，不會重複新增。"""
    try:
        existing_client_names = {c.name for c in Client.query.with_entities(Client.name).all()}
        for name in DEFAULT_CLIENTS:
            if name not in existing_client_names:
                db.session.add(Client(name=name))
        existing_editor_names = {e.name for e in Editor.query.with_entities(Editor.name).all()}
        for name in DEFAULT_EDITORS:
            if name not in existing_editor_names:
...
def get_dashboard():
    ...
    # 若客戶或剪輯手為空，先補齊預設名單再查（避免新 DB / 啟動競態導致畫面空白）
    ...
    if not clients_active or not editors_active:
        seed_defaults()
        clients_active = Client.query.filter(text("clients.archived = 0")).order_by(Client.id).all()
        editors_active = Editor.query.filter(text("editors.archived = 0")).order_by(Editor.id).all()
