...
- 環境變數 **`SITE_PASSWORD`**：若設定，進入網站前會先要求輸入此密碼才能看到登入頁或首頁。
- **未設定**時：不啟用此層，直接進入登入/首頁流程。
- 程式預設為 `90010198`（可在 Render 後台用 `SITE_PASSWORD` 覆蓋）。
- **密碼 `guest`**：進入 **Demo 檢視版**（`/viewer`），與主站相同的填寫與操作介面，但：
  - **年度目標**為 **1000 萬**；
  - **客戶名稱**、**剪輯人員**以代號顯示（例如「客戶 1」「剪輯 1」），可自由新增；
  - **底迪帳號**頁面不變，但資料不與後端連動；
  - 所有資料僅存在記憶體，**不寫入後端也不寫入 localStorage**，關閉視窗或重新整理即還原為初始狀態（無痕模式）。
